...
CKinectFusionColorBasics::CKinectFusionColorBasics() :
    ...
    m_bTranslateResetPoseByMinDepthThreshold(false),
...
{
    InitializeCriticalSection(&m_lockFusionProcess);
    ...
    m_reconstructionParams.voxelCountZ = 256;   // This will require a GPU with at least 512MB
...
CKinectFusionColorBasics::~CKinectFusionColorBasics()
{
	...
	DeleteCriticalSection(&m_lockFusionProcess);
}
...
LRESULT CALLBACK CKinectFusionColorBasics::DlgProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    switch (message)
    {
        ...
        case WM_COMMAND:
			...
			if (IDC_BUTTON_CREATE_MESH == LOWORD(wParam) && BN_CLICKED == HIWORD(wParam))
			{
				SetStatusMessage(L"Creating and saving mesh of reconstruction, please wait...");

				EnterCriticalSection(&m_lockFusionProcess);
				...
				LeaveCriticalSection(&m_lockFusionProcess);
			}
            break;
    }
...
HRESULT CKinectFusionColorBasics::InitializeKinectFusion()
{
	...
	hr = NuiFusionCreateImageFrame(NUI_FUSION_IMAGE_TYPE_POINT_CLOUD,m_cDepthWidth,m_cDepthHeight,nullptr,&m_pPointCloud_depth);
	...
	else
	{
		...
		if(m_imageSaveInterval == 0)
		{
			...
			for(UINT y=0;y<matheight;y++)
			{
				...
				{
					int cory = colorpoints[y*matwidth+x].y;
					int corx = colorpoints[y*matwidth+x].x;

					if(cory>=0 && cory<m_cColorHeight && corx>=0 && corx<m_cColorWidth)
					{
						savemat.at<cv::Vec3b>(y,x)[0] = static_cast<uchar>(pBuffer[cory*pitch + corx*4]);
						savemat.at<cv::Vec3b>(y,x)[1] = static_cast<uchar>(pBuffer[cory*pitch + corx*4 + 1]);
...
/// Create a color mesh
/// </summary>
HRESULT CKinectFusionColorBasics::SaveMeshFile(INuiFusionColorMesh* pMesh, std::string &meshFileName)
{
	HRESULT hr = S_OK;

	DWORD nX,nY;
	NuiImageResolutionToSize(m_depthImageResolution,nX,nY);

	//save point cloud and texture images
	SetStatusMessage(L"Saving texture images...");
	for(int i=0;i<externalPara.size();i++)
	{
		char buffer[100];
		sprintf(buffer,"%s%03d.txt",pathprefix_geometry,i);
		ofstream camPara(buffer);
		camPara<<externalPara[i].M11<<' '<<externalPara[i].M12<<' '<<externalPara[i].M13<<' '<<externalPara[i].M14<<endl;
		camPara<<externalPara[i].M21<<' '<<externalPara[i].M22<<' '<<externalPara[i].M23<<' '<<externalPara[i].M24<<endl;
		camPara<<externalPara[i].M31<<' '<<externalPara[i].M32<<' '<<externalPara[i].M33<<' '<<externalPara[i].M34<<endl;
		camPara<<externalPara[i].M41<<' '<<externalPara[i].M42<<' '<<externalPara[i].M43<<' '<<externalPara[i].M44<<endl;
	}

	for(int i=0;i<saveImages_color.size();i++)
	{
		char buffer[100];
	...
	{
		char buffer[100];
		sprintf(buffer,"%s_pt%03d.off",pathprefix_depth,i);
		ofstream pointcloud(buffer);
		pointcloud<<"OFF"<<endl;
		pointcloud<<worldpoints[i].size()<<' '<<0<<" 0"<<endl;
		for(int j=0;j<worldpoints[i].size();j++)
			pointcloud<<worldpoints[i][j].x<<' '<<worldpoints[i][j].y<<' '<<worldpoints[i][j].z<<endl;
		pointcloud.close();
	}*/
	...
	unsigned int numVertices = pMesh->VertexCount();
	unsigned int numTriangleIndices = pMesh->TriangleVertexIndexCount();
	unsigned int numTriangles = numVertices / 3;
	unsigned int numColors = pMesh->ColorCount();
	...
	hr = pMesh->GetTriangleIndices(&triangleIndices);
	if (FAILED(hr))
	{
		return hr;
	}
	...
	FILE *meshFile = NULL;
	errno_t err = fopen_s(&meshFile, meshFileName.c_str(), "w");
	...
	{
		//fprintf_s(meshFile, "%f %f %f\n", vertices[t].x, vertices[t].y, vertices[t].z);
		fprintf_s(meshFile, "%f %f %f %d %d %d 255\n", vertices[t].x, vertices[t].y, vertices[t].z, (colors[t]>>16)&255, (colors[t]>>8)&255, colors[t]&255);
	}

	for (unsigned int t=0; t<numTriangles; ++t)
	{
		fprintf_s(meshFile, "3 %d %d %d\n", triangleIndices[3*t+0], triangleIndices[3*t+1], triangleIndices[3*t+2]);
	}
